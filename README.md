...
|     [![Board][board-src]](plugins/boardBulkStatus/index.ts)      | Select multiple cards with `Cmd/Ctrl + click` or `Shift + click`, then drag any one of them to move the whole selection to another status.                                                   | ☑️  | 👆  |
